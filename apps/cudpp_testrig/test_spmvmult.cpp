...
int
testSparseMatrixVectorMultiplyDouble(int argc, const char** argv) 
{
    cudpp_app::StopWatch timer;
...
*/
    ...
    float * reference = (float *) malloc(sizeof(float) * rows);
    double * A = (double *) malloc(sizeof(double ) * entries);
    unsigned int * indx = 
        (unsigned int *) malloc(sizeof(unsigned int) * entries);
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_y, rows * sizeof(double))); 

    CUDA_SAFE_CALL(cudaMemcpy(d_x, x, cols * sizeof(double),
                              cudaMemcpyHostToDevice));
    CUDA_SAFE_CALL(cudaMemcpy(d_y, y, rows * sizeof(double),
                              cudaMemcpyHostToDevice));
    ...
    // Compute gold comparison
    float * f_x = (float *) malloc(sizeof(float) * cols);
	for(int i=0;i<cols;i++)
		f_x[i]=(float)x[i];
    sparseMatrixVectorMultiplyGold(&m, f_x, reference);
    ...
    CUDA_SAFE_CALL(cudaMemcpy(y, d_y, rows * sizeof(double),
                              cudaMemcpyDeviceToHost));

    float * f_y = (float *) malloc(sizeof(float) * rows);
	for(int i=0;i<rows;i++)
		f_y[i]=(float)y[i];
    // epsilon is 0.001f, answer must be within epsilon
    bool spmv_result = compareArrays(reference, f_y, rows, 0.001f);
    retval += spmv_result ? 0 : 1;
